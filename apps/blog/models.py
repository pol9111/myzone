...
class Tag(models.Model):
    ...
    def get_absolute_url(self): # 获得路径
        return reverse('blog:tag', kwargs={'slug': self.slug}) # urls那边路径
...
class Article(models.Model):
    ...
    author = models.ForeignKey(settings.AUTH_USER_MODEL, verbose_name='作者') # 还有个外键
    title = models.CharField(max_length=150, verbose_name='文章标题')
    summary = models.TextField('文章摘要', max_length=230, default='文章摘要等同于网页description内容，请务必填写...')
    body = models.TextField(verbose_name='文章内容')
    img_link = models.CharField('图片地址', default=IMG_LINK, max_length=255)
    create_date = models.DateTimeField(verbose_name='创建时间', auto_now_add=True) # 只加一次
    update_date = models.DateTimeField(verbose_name='修改时间', auto_now=True) # 每次都加
    ...
    def get_absolute_url(self):
        return reverse('blog:detail', kwargs={'slug': self.slug}) # urls那边路径

    def body_to_markdown(self): # 文章具体内容
        return markdown.markdown(self.body, extensions=[
            'markdown.extensions.extra',
            'markdown.extensions.codehilite',
        ])

    def update_views(self): # 加一阅读量
        self.views += 1
        self.save(update_fields=['views'])

    def get_pre(self): # 上一篇文章
        return Article.objects.filter(id__lt=self.id).order_by('-id').first()

    def get_next(self): # 下一篇文章
...
class Carousel(models.Model):
    ...
    class Meta:
        verbose_name = '图片轮播'
        verbose_name_plural = verbose_name
        ordering = ['number', '-id'] # 编号越小越靠前，添加的时间约晚约靠前
...
class FriendLink(models.Model):
    ...
    logo = models.URLField('网站LOGO', blank=True, help_text='请填写http或https开头的完整形式地址')
    ...
    def active_to_false(self): # 调用方法改成无效
        self.is_active = False
        self.save(update_fields=['is_active'])

    def show_to_false(self):# 调用方法改成展现首页
        self.is_show = True
