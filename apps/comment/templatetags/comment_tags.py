...
def get_comment_count(entry):
    '''获取一个文章的评论总数'''
    lis = entry.article_comments.all() # 都是表里字段的别名 belong
    return lis.count()
...
def get_child_comments(com):
    '''获取一个父评论的子评论列表'''
    lis = com.articlecomment_child_comments.all() # parent
    return lis
...
def get_comment_user_count(entry):
    '''获取评论人总数'''
    p = []
    lis = entry.article_comments.all() # belong
    for each in lis:
...
def get_notifications(user,f=None):
    '''获取一个用户的对应条件下的提示信息'''
    if f=='true':
        lis = user.notification_get.filter(is_read=True) # get_p
    elif f=='false':
